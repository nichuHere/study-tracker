 ...
 *
 * Exposes tools so any MCP client (Claude Desktop, VS Code Copilot, etc.)
 * can manage tasks, subjects, exams, and reminders in the StudyTracker app.
 *
 * Usage:
 *   "add a task for malayalam for 20 mins"
 *   "show today's tasks for Adhi"
 *   "add a reminder for science project due March 10"
 */
...
async function resolveProfileId(profileName) {
  if (!profileName) {
    // If no name given, return the first profile
...
server.tool(
  ...
  async ({ subject, duration, date, chapter, activity, instructions, profile_name }) => {
    const profile = await resolveProfileId(profile_name);
...
server.tool(
  ...
  async ({ date, profile_name, include_completed }) => {
    ...
    if (data.length === 0) {
      return { content: [{ type: 'text', text: `No ${include_completed ? '' : 'pending '}tasks for ${profile.name} on ${taskDate}.` }] };
    }
    ...
    return {
      content: [{
        type: 'text',
        text: `Tasks for ${profile.name} on ${taskDate}:\n${lines.join('\n')}`,
      }],
    };
  }
);
...
server.tool(
  ...
  async ({ task_id }) => {
    ...
    return {
      content: [{
        type: 'text',
        text: `✅ Marked task "${data.subject}${data.chapter ? ` — ${data.chapter}` : ''}" as completed.`,
      }],
    };
  }
);
...
server.tool(
  ...
  async ({ profile_name }) => {
    ...
    if (data.length === 0) {
      return { content: [{ type: 'text', text: `No subjects found for ${profile.name}.` }] };
    }
    ...
    return {
      content: [{
        type: 'text',
        text: `Subjects for ${profile.name}:\n${lines.join('\n')}`,
      }],
    };
  }
);
...
server.tool(
  ...
  async ({ name, chapters, profile_name }) => {
    ...
    const chapterObjs = (chapters || []).map(ch => ({
      name: ch,
      status: 'pending',
    }));
    ...
    return {
      content: [{
        type: 'text',
        text: `📚 Subject "${name}" added for ${profile.name} with ${chapterObjs.length} chapters. (ID: ${data.id})`,
      }],
    };
  }
);
...
server.tool(
  ...
  async ({ title, date, description, profile_name }) => {
    ...
    return {
      content: [{
        type: 'text',
        text: `🔔 Reminder added for ${profile.name}:\n• ${title}\n• Date: ${date}${description ? `\n• Details: ${description}` : ''}\n• ID: ${data.id}`,
      }],
    };
  }
);
...
server.tool(
  ...
  async ({ date, profile_name }) => {
    ...
    const { data: oneTime, error: e1 } = await supabase
      .from('reminders')
      .select('*')
      .eq('profile_id', profile.id)
      .eq('date', targetDate);

    // Get recurring reminders for the day of week
    ...
    if (lines.length === 0) {
      return { content: [{ type: 'text', text: `No reminders for ${profile.name} on ${targetDate}.` }] };
    }

    return {
      content: [{
        type: 'text',
        text: `Reminders for ${profile.name} on ${targetDate}:\n${lines.join('\n')}`,
      }],
    };
  }
);

// ─── Tool: add_recurring_reminder ───────────────────────────────────────────
...
server.tool(
  ...
  async ({ title, time, end_time, days, description, profile_name }) => {
    ...
    return {
      content: [{
        type: 'text',
        text: `🔁 Recurring reminder added for ${profile.name}:\n• ${title}\n• ${time}–${end_time || '20:00'} on ${dayStr}\n• ID: ${data.id}`,
      }],
    };
  }
);
...
server.tool(
  ...
  async ({ profile_name }) => {
    ...
    if (data.length === 0) {
      return { content: [{ type: 'text', text: `No exams found for ${profile.name}.` }] };
    }
